...
# GET all tasks (with aggregated todo stats)
@task_bp.get("")
@jwt_required()
def get_tasks():
    rows = TaskService.get_all_with_stats()
    result = []

    for task, total_todos, completed_todos in rows:
        dto = task_to_dto(task)
        dto["total_todos"] = int(total_todos or 0)
        dto["completed_todos"] = int(completed_todos or 0)
        result.append(dto)
