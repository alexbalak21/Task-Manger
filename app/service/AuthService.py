...
class AuthService:
    ...
    @staticmethod
    def register(name, email, password, invite_code, profile_image=None):
        """Public self-registration. Always creates a 'member', and requires a
        valid team invite code generated by a manager (or admin) for a team."""
        if UserRepository.find_by_email(email):
            return None, "Email already registered"

        from service.TeamService import TeamService
        team, error = TeamService.redeem_invite_code(invite_code)
        if error:
            return None, error
        ...
        TeamMemberRepository.save(TeamMember(team_id=team.id, user_id=user.id))
